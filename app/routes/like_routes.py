...
def like_app():
    try:
        ...
        if not auth_token:
            return clear_auth_cookies(make_response(jsonify({'error': 'Not authenticated'}), 401))
...
def unlike_app():
    try:
        ...
        if app:
            deleted = (
                LikedApp.delete().where(LikedApp.account == account, LikedApp.app == app).execute()
            )
            if deleted:
...
def get_liked_apps():
    ...
    account = Account.get_or_none(Account.sign_in_token == auth_token)
    if not account:
        return clear_auth_cookies(make_response(jsonify({'error': 'Invalid authentication'}), 401))
